...
: Node("kiapi_infra_static_tf_node", options)
{
  ...
  // //using vector_map
  // this->declare_parameter("use_vector_map", "");
  // is_map = (bool)this->get_parameter("use_vector_map").as_string();
  if (is_map){
      map_path = declare_parameter<std::string>("map_projector_info_path", "");
      ReadMapYaml();
    //   subscription_mapinfo_ = this->create_subscription<autoware_map_msgs::msg::MapProjectorInfo>(
    // "/map/map_projector_info", 10,
    // std::bind(&InfraStaticTFNode::MapinfoCallback, this, std::placeholders::_1));
  }
  ...
  timer_ = this->create_wall_timer(
      period, std::bind(&InfraStaticTFNode::timer_callback, this));
}
